...
static void dumpnand_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(dumpData->finished) {
        ...
        if(R_SUCCEEDED(dumpData->result)) {
            prompt_display_notify("成功", "已导出 NAND。", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void dumpnand_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("正在导出 NAND", "按 B 取消。", true, data, dumpnand_update, NULL);
        } else {
            error_display_res(NULL, NULL, res, "无法初始化导出 NAND。");
            free(data);
        }
    } else {
        free(data);
...
void dumpnand_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "无法分配导出 NAND 数据。");
