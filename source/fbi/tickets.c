...
static list_item delete_ticket = {"删除关联表", COLOR_TEXT, action_delete_ticket};
static list_item delete_unused_tickets = {"删除未使用的关联表", COLOR_TEXT, action_delete_tickets_unused};
...
static void tickets_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "无法分配关联表操作的数据.");
    ...
    list_display("关联表操作", "A: 选择, B: 返回", data, tickets_action_update, tickets_action_draw_top);
}
...
static void tickets_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "无法初始化关联表目录结构.");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "无法列举关联表目录.");
...
void tickets_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "无法分配关联表数据.");
    ...
    list_display("关联表", "A: 选择, B: 返回, X: 刷新", data, tickets_update, tickets_draw_top);
}
